...
protected:
    ...
    std::unique_ptr<IAllocator> CreatePlasmaAllocator(int64_t memory_limit) {
        return std::make_unique<PlasmaAllocator>(
            plasma_dir_->string(),
...
#ifdef JEMALLOC_AVAILABLE
        return std::make_unique<JemallocAllocator>(
            plasma_dir_->string(),
            fallback_dir_->string(),
            false,  // hugepages_enabled
            memory_limit
    ...
    double CalculateFragmentationRatio(IAllocator& allocator,
        ...
        if (test_size <= kKB) return 1.0;  // Highly fragmented
    ...
    void PrintResult(const BenchmarkResult& result) {
        std::cout << "=== " << result.test_name << " [" << result.allocator_type << "] ===" << std::endl;
        std::cout << "Object Size: " << (result.object_size / kKB) << " KB" << std::endl;
        std::cout << "Memory Limit: " << (result.memory_limit / kMB) << " MB" << std::endl;
        std::cout << "Allocation Rate: " << result.alloc_ops_per_sec << " ops/sec" << std::endl;
        std::cout << "Free Rate: " << result.free_ops_per_sec << " ops/sec" << std::endl;
        std::cout << "Avg Alloc Latency: " << result.avg_alloc_latency_us << " μs" << std::endl;
        std::cout << "P95 Alloc Latency: " << result.p95_alloc_latency_us << " μs" << std::endl;
        std::cout << "Fragmentation Ratio: " << (result.fragmentation_ratio * 100) << "%" << std::endl;
        std::cout << "Memory Utilization: " << (result.memory_utilization * 100) << "%" << std::endl;
...
public:
    BenchmarkResult RunBenchmark(std::unique_ptr<IAllocator> allocator,
        ...
        auto result = CreateResult("Sequential_Allocation", allocator_type,
                                 object_size, memory_limit);
        ...
        result.memory_utilization = static_cast<double>(allocator->Allocated()) / memory_limit;
        ...
        result.test_duration_sec = duration_cast<microseconds>(end_time - start_time).count() / 1e6;

        // Calculate performance metrics
        if (!alloc_latencies.empty()) {
            result.avg_alloc_latency_us = std::accumulate(alloc_latencies.begin(),
                                                        alloc_latencies.end(), 0.0) / alloc_latencies.size();
        ...
        if (!free_latencies.empty()) {
            result.avg_free_latency_us = std::accumulate(free_latencies.begin(),
                                                       free_latencies.end(), 0.0) / free_latencies.size();
...
public:
    BenchmarkResult RunBenchmark(std::unique_ptr<IAllocator> allocator,
        ...
        auto result = CreateResult("Mixed_Allocation", allocator_type,
                                 object_size, memory_limit);
        ...
        std::vector<size_t> sizes = {object_size, object_size/2, object_size*2, object_size/4};
        ...
        for (int i = 0; i < iterations; ++i) {
            ...
            double alloc_probability = active_allocations.empty() ? 1.0 :
                                     std::max(0.3, 1.0 - static_cast<double>(active_allocations.size()) / 100.0);
        ...
        result.fragmentation_ratio = CalculateFragmentationRatio(*allocator, active_allocations);
        result.memory_utilization = static_cast<double>(allocator->Allocated()) / memory_limit;
...
public:
    BenchmarkResult RunCheckerboardPattern(std::unique_ptr<IAllocator> allocator,
        ...
        auto result = CreateResult("Checkerboard_Pattern", allocator_type,
                                 object_size, memory_limit);
        ...
        int max_objects = static_cast<int>(memory_limit / (object_size * 2));  // Leave room for gaps
    ...
    BenchmarkResult RunBurstAllocation(std::unique_ptr<IAllocator> allocator,
        ...
        auto result = CreateResult("Burst_Allocation", allocator_type,
                                 object_size, memory_limit);
...
TEST_F(SequentialAllocationBenchmark, CompareAllocators_SmallObjects) {
    size_t object_size = 100 * kKB;
    int64_t memory_limit = 512 * kMB;

    std::cout << "\n=== Sequential Allocation Benchmark - Small Objects ===\n" << std::endl;

    // Test PlasmaAllocator (dlmalloc)
    auto plasma_allocator = CreatePlasmaAllocator(memory_limit);
    auto plasma_result = RunBenchmark(std::move(plasma_allocator), "dlmalloc",
                                     object_size, memory_limit);
    ...
    if (jemalloc_allocator) {
        auto jemalloc_result = RunBenchmark(std::move(jemalloc_allocator), "jemalloc",
                                           object_size, memory_limit);
        ...
        std::cout << "Fragmentation Reduction: "
                  << ((plasma_result.fragmentation_ratio - jemalloc_result.fragmentation_ratio) * 100)
                  << "% (negative = jemalloc better)" << std::endl;
        std::cout << "Allocation Speed Improvement: "
                  << ((jemalloc_result.alloc_ops_per_sec / plasma_result.alloc_ops_per_sec - 1.0) * 100)
                  << "%" << std::endl;
    } else {
...
TEST_F(SequentialAllocationBenchmark, CompareAllocators_LargeObjects) {
    ...
    auto plasma_allocator = CreatePlasmaAllocator(memory_limit);
    auto plasma_result = RunBenchmark(std::move(plasma_allocator), "dlmalloc",
                                     object_size, memory_limit, 100);  // Fewer iterations for large objects
    PrintResult(plasma_result);
...
TEST_F(PathologicalFragmentationBenchmark, CompareAllocators_CheckerboardPattern) {
    ...
    // Test PlasmaAllocator (dlmalloc)
    auto plasma_allocator = CreatePlasmaAllocator(memory_limit);
    auto plasma_result = RunCheckerboardPattern(std::move(plasma_allocator), "dlmalloc",
                                               object_size, memory_limit);
    PrintResult(plasma_result);
    ...
    auto jemalloc_allocator = CreateJemallocAllocator(memory_limit);
    if (jemalloc_allocator) {
        auto jemalloc_result = RunCheckerboardPattern(std::move(jemalloc_allocator), "jemalloc",
                                                     object_size, memory_limit);
        PrintResult(jemalloc_result);
        ...
        std::cout << "Successful Large Alloc Difference: "
                  << (jemalloc_result.successful_allocations - plasma_result.successful_allocations)
...
TEST_F(PathologicalFragmentationBenchmark, CompareAllocators_BurstPattern) {
    ...
    // Test PlasmaAllocator (dlmalloc)
    auto plasma_allocator = CreatePlasmaAllocator(memory_limit);
    auto plasma_result = RunBurstAllocation(std::move(plasma_allocator), "dlmalloc",
                                           object_size, memory_limit);
    PrintResult(plasma_result);
    ...
    auto jemalloc_allocator = CreateJemallocAllocator(memory_limit);
    if (jemalloc_allocator) {
        auto jemalloc_result = RunBurstAllocation(std::move(jemalloc_allocator), "jemalloc",
                                                 object_size, memory_limit);
        PrintResult(jemalloc_result);
        ...
        std::cout << "Burst Allocation Speed Improvement: "
                  << ((jemalloc_result.alloc_ops_per_sec / plasma_result.alloc_ops_per_sec - 1.0) * 100)
                  << "%" << std::endl;
        std::cout << "Memory Utilization Improvement: "
