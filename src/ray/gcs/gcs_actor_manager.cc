...
void GcsActorManager::DestroyActor(const ActorID &actor_id,
  ...
  if (actor->GetState() == rpc::ActorTableData::DEAD) {
    ...
    if (mutable_actor_table_data->death_cause().context_case() ==
        ...
        mutable_actor_table_data->death_cause().actor_died_error_context().reason() ==
      ...
      gcs_table_storage_->ActorTable().Put(
          actor->GetActorID(), *mutable_actor_table_data, {[](auto) {}, io_context_});
    }
    if (done_callback) {
      done_callback();
    }
      ...
      if (!force_kill && graceful_shutdown_timeout_ms > 0) {
        ...
        timer->async_wait(
            [this, actor_id, worker_id, death_cause, timer, graceful_shutdown_timeout_ms](
              ...
              // Cleanup will happen in RestartActor when worker reports death.
  ...
  if (!is_restartable && !defer_cleanup) {
...
void GcsActorManager::RestartActor(const ActorID &actor_id,
  ...
  if (remaining_restarts != 0 ||
    ...
    // Clean up state for non-restartable actors (deferred or not).
    ...
    if (registered_iter != registered_actors_.end()) {
      auto actor_to_cleanup = registered_iter->second;  // Save before erase
